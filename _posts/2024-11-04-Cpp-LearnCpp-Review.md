...
在C++中，类的内存大小（sizeof）取决于多个因素，包括其成员变量的类型、虚函数表指针（如果有虚函数的话）、继承方式（如是否有多重继承）、以及编译器优化和内存对齐等。以下是一些关注点：

空类：空类（不包含任何成员变量的类）同样可以被实例化，为保证每个实例在内存中都有独一无二的地址，空类的大小通常被编译器设置为 1 字节。